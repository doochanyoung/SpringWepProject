  ...
  ============================-->
		...
		<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
			...
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav nav-menu">
					...
					<li class="nav-item menu-active"><a href="/dataroom/dataroomList">Data Room</a></li>
	...
	<section id="boards">
		<div class="container">
					...
					<div class="row">
						<div class="col-7">
							<div class="form-group">
								<input type="text" class="form-control" name="dataroomSearch"
			...
			<div class="row">
				<table class="table table-striped table-sm"
					...
					<tbody>
						<c:forEach items="${list}" var="dataroomVO">
							<tr>
								<th class="mobile" scope="row">${dataroomVO.dataroomId }</th>
								<td><a href='/dataroom/dataroomRead${pageMaker.makeSearch(pageMaker.pageHandler.page)}&dataroomId=${dataroomVO.dataroomId}'><c:if test="${dataroomVO.dataroomIsReply }"><i class="fab fa-replyd"></i></c:if>${dataroomVO.dataroomTitle } <strong>[${dataroomVO.dataroomCommCnt}]</strong></a></td>
								<td class="mobile">${dataroomVO.dataroomUserId }</td>
								<td class="mobile"><fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${dataroomVO.dataroomRegdate }"/></td>
								<td class="mobile">${dataroomVO.dataroomHit }</td>
								<td class="mobile">${dataroomVO.dataroomLike }</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
				<div class="text-center">
					...
					<button class="btn btn-default float-right m-2" type="button" id="dataroomWrite">글 작성</button>
					</c:if>
				</div>
			</div>
		</div>
		<nav>
			<ul class="pagination pagination-info justify-content-center">
				...
				<li class="page-item"><a class="page-link" href="dataroomList${pageMaker.makeSearch(pageMaker.startPage - 1) }" aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
						<span class="sr-only">Previous</span></a></li>
				</c:if>
				<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
					<li class = "page-item <c:out value="${pageMaker.pageHandler.page == idx ? 'active' : ''}" />">
					<a class="page-link" href="dataroomList${pageMaker.makeSearch(idx)}">${idx }</a>
					</li>
				</c:forEach>
				<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
				<li class="page-item"><a class="page-link" href="dataroomList${pageMaker.makeSearch(pageMaker.endPage + 1) }" aria-label="Next"> <span aria-hidden="true">&raquo;</span> <span
	...
	<script>
		$(document).ready(function() {
			$('#dataroomWrite').on("click", function(ext) {
				self.location = "dataroomWrite";
			});
			$("#searchButton").on("click", function(event){
				self.location = "dataroomList" + '${pageMaker.makeQuery(1)}' + "&searchType=" + $("#searchType option:selected").val()
